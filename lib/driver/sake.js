...
(function () {
    ...
    function defineTask (TaskClass, name, deps, fn) {
        ...
        if (!task) {
            task = new SakeDriver[TaskClass](name, deps, fn);
    ...
    SakeDriverProto = Object.create(Driver, {
        runSakefile: {
            value: function (path) {
    ...
    module.exports = SakeDriver = Proteus.create(SakeDriverProto, {
    ...
    // Make SakeDriver an event emitter
    Proteus.extend(SakeDriver, require("events").EventEmitter.prototype, {
        namespace: runDriverInNamespace.bind(SakeDriver, SakeDriver),
        stitch: function () {
            ...
            runDriverInNamespace.apply(this, [StitchDriver].concat(args))
        }
    });
