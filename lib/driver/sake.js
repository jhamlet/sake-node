...
(function () {
    ...
    module.exports = sake = Proteus.create(sakeProto, {
        ...
        async: function (name, deps, fn) {
            var task = defineTask("Task", name, deps, fn);
            task.async = true;
            return task;
        },
        
        atask: util.aliasMethod("async"),
        
        file: defineTask.bind(sake, "FileTask"),

        fileCreate: defineTask.bind(sake, "FileCreateTask"),

        directory: function (name, deps, action) {
            var task =  sake.fileCreate(name, function (t) {
                    if (!Path.existsSync(t.name)) {
                        sake.mkdir_p(t.name);
                    }
                });
            
            task.enhance(deps, action);
