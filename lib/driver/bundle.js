...
(function () {
    ...
    function composeTask (name, type, deps, action) {
        ...
        sake.task(taskName, deps);
        sake.task(modeName, [taskName], action);
        sake.task(name + "-" + type.extension, [modeName]);
    }
    
    traceTask = composeTask.bind(this, "trace");
    weaveTask = composeTask.bind(this, "weave");
    //------------------------------------------------------------------------
    // PUBLIC
    //------------------------------------------------------------------------
    module.exports = bundler = Proteus.create(Driver, {
        ...
        run: function (name, fn) {
            ...
            task = task || sake.task(name, function (t, ctx) {
                // run the bundle specific task for the current mode, and type.
                sake.task(t.name + "." + ctx.type.extension).invoke(ctx);
            });
