...
(function () {
    ...
    function bundleTaskName (bundle, type) {
        return bundle.name + "." + type.extension;
    }
    
    function modeTaskName (name, bundle, type) {
        return name + "-" + bundleTaskName(bundle, type);
    }
    
    function composeTask (name, type, deps, action) {
        var taskName = bundleTaskName(CURRENT_BUNDLE, type),
            modeName = modeTaskName(name, CURRENT_BUNDLE, type)
        ;
        
        if (!action && typeof deps === "function") {
            action = deps;
            deps = null;
        }

        sake.task(modeName, deps, action);
        sake.task(name + "-" + type.extension, [modeName]);
        // sake.task(name + "-" + type.extension, [modeName]);
    }
    
    function traceTask (bundle, type) {
        var taskname = modeTaskName("trace", bundle, type),
            task
        ;
        
        if (!(task = sake.Task.get(taskname))) {
            task = sake.task(taskname);
        }
        
        return task;
    }
    
    function bundleTempPath (bundle, type) {
        var tmpDir  = Path.join(process.cwd(), sake.options.stitchTempDirectory),
            tmpFile = bundleTaskName(CURRENT_BUNDLE, type)
        ;
        
        return Path.join(tmpDir, tmpFile);
    }
    
    function bundleTempTask (bundle, type) {
        var tmpPath = bundleTempPath(bundle, type),
            task
        ;
        
        task = sake.Task.get(tmpPath) ||
            sake.file(tmpPath, function (t, ctx) {
                console.log("compiling " + t.name );
                t.startAsync();
                sake.sh(
                    "cat " + t.prerequisites.join(" ") + " > " + tmpPath,
                    t.clearAsync
                );
            });
    ...
    function weaveTask (bundle, type) {
        var taskname = modeTaskName("weave", bundle, type),
            temptask = bundleTempTask(bundle, type),
            task
        ;
        
        task = sake.Task.get(taskname) ||
            sake.task(taskname, [temptask.name], function (t, ctx) {
                ctx.write(FS.readFileSync(t.prerequisites[0]));
            });
    ...
    module.exports = bundler = Proteus.create(Driver, {
        ...
        get task () {
            ...
            if ((type = this.__currentType__)) {
                return sake.Task.get(CURRENT_BUNDLE.name + "." + type.extension);
            }
        ...
        read: function (type, path) {
            ...
            traceTask(CURRENT_BUNDLE, type).addAction(function (t, ctx) {
                ctx.write("file: " + path + "\n");
            });
            
            tmpTask = bundleTempTask(CURRENT_BUNDLE, type).addPrerequisite(
                Path.join(process.cwd(), path)
            );
            
            sake.task(modeTaskName("weave", CURRENT_BUNDLE, type), [tmpTask.name]);
        ...
        insert: function (type, val) {
            ...
            traceTask(type, function (t, ctx) {
                ctx.write("content: " + val + "\n");
            });
            
            weaveTask(type, function (t, ctx) {
        ...
        exec: function (type, cmd) {
            ...
            weaveTask(type, function (t, ctx) {
                ...
                sake.sh(cmd, function (result) {
                    sake.log("cmd: " + result.slice(0, 64) + "...");
                    ctx.write(result + "\n");
                    t.complete();
                });
            });
        ...
        run: function (name, fn) {
            ...
            if (!task) {
                ...
                task = sake.task(name, function (t, ctx) {
                    var bundleCtx = new Context();
                    // run the bundle specific task for the current mode,
                    // and type.
                    sake.task(
                        modeTaskName(ctx.mode, task, ctx.type)
                    ).invoke(bundleCtx);
                    
                    task.on("complete", function () {
                        console.log(bundleCtx.source);
