...
(function () {
    ...
    module.exports = BundleDriver = Proteus.create(Driver, {
        ...
        include: function (name) {
            var ctx = CURRENT_BUNDLE,
                bndl = SakeDriver.task(name),
                preamble
            ;
        ...
        insert: function (type, val) {
            ...
            task = SakeDriver.task(CURRENT_BUNDLE.name);
            
            SakeDriver.task(task, function (t, ctx) {
                if (ctx.type !== type) {
                    return;
                }
        ...
        exec: function (type, cmd) {
            ...
            SakeDriver.task(CURRENT_BUNDLE.name, function (t, ctx) {
                if (ctx.type !== type) {
                    return;
                }
        ...
        run: function (name, fn) {
            ...
            CURRENT_BUNDLE = SakeDriver.task(name);
