...
function updateBindings (type) {
    [type.name].concat(type.extensions).forEach(function (m) {
        if (!this[name]) {
            this[name] = bindType(type);
        }
    }, BundleDriver);
    
}

function renameBindings (type, oldname, newname) {
    [oldname].concat(type.extensions).forEach(function (m) {
        delete this[m];
        this[name] = bindType(type);
    }, BundleDriver);
}
...
module.exports = BundleDriver = Proteus.create(Driver, {
    ...
    read: function (type, path) {
        ...
        dep  = new FileTask(path);
        new Task(ctx.name, path, function (t, ctx) {
            if (ctx.type !== type) {
                return;
            }
    ...
    insert: function (type, val) {
        ...
        new Task(BundleDriver.context.name, function (t, ctx) {
            if (ctx.type !== type) {
                return;
            }
     ...
     * 
     * @method comment
     * @param type {object|string} a TypeModel or type string "text/javascript",
     *          "javascript", "js", etc..
        ...
        new Task(BundleDriver.context.name, function (t, ctx) {
            ...
            switch (ctx.command) {
                case "trace":
                    ...
                    break;
                case "weave":
                    StitchDriver.sh(cmd, function (result) {
...
TypeModel.on("nameChanged", renameBindings);
