...
(function () {
    ...
    var Proteus      = require("proteus"),
        ...
        bundler,
        CURRENT_BUNDLE,
        traceTask, weaveTask, weaveAsyncTask
    ;
    ...
    function composeTask (name, taskType, type, deps, action) {
        var taskName = bundleTaskName(name, CURRENT_BUNDLE, type);
        ...
        sake[taskType](taskName, deps, action);
        sake.task(name + "-" + type.extension, [taskName]);
    }
    
    traceTask = composeTask.bind(this, "trace", "task");
    weaveTask = composeTask.bind(this, "weave", "task");
    //------------------------------------------------------------------------
    // PUBLIC
    //------------------------------------------------------------------------
    module.exports = bundler = Proteus.create(Driver, {
        ...
        run: function (name, fn) {
            ...
            task = task || sake.task(name, function (t, ctx) {
                sake.task(bundleTaskName(ctx.mode, task, ctx.type)).invoke(ctx);
            });
