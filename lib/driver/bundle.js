...
(function () {
    ...
    function composeBundle (
        type,
        traceDeps, traceFn, traceAsync,
        weaveDeps, weaveFn, weaveAsync
    ) {
        var traceName = bundleTaskName("trace", CURRENT_BUNDLE, type),
            weaveName = bundleTaskName("weave", CURRENT_BUNDLE, type)
        ;
        
        sake.task(traceName, traceDeps, traceFn, traceAsync);
        sake.task(weaveName, weaveDeps, weaveFn, weaveAsync);
