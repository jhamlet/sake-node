...
(function () {
    ...
    function getCurrentStitchTempTask (type) {
        getStitchTask(CURRENT_BUNDLE, type);
    ...
    function getContentTask (bundle, type, val) {
        var tmpDir = sake.options.stitchTempDirectory,
            filename, filepath, dirpath, task
        ;
        ...
        filename = bundle.name + "-content-" + bundle.contentCount + "." + type.extension;
        filepath = Path.join(tmpDir, filename);
        dirpath  = Path.dirname(filepath);
        
        sake.directory(dirpath, [tmpDir]);
    ...
    module.exports = bundler = Proteus.create(Driver, {
        ...
        read: function (type, path) {
            ...
            getCurrentStitchTempTask(type).addPrerequisite(path);
        ...
        insert: function (type, val) {
            ...
            getCurrentTraceTask(type, function (t, ctx) {
                ctx.write("content: " + val + "\n");
            });
            
            tmptask = getCurrentContentTask(type, val);
            getCurrentStitchTempTask(type).addPrerequisite(tmptask.name);
        ...
        exec: function (type, cmd) {
            ...
            // getCurrentStitchTempTask(type).(type, function (t, ctx) {
            //     t.begin();
