...
(function () {
    ...
    function defineTask (name) {
        sake.task(name, taskAction(name), false);
        sake.task(name, render, false);
    }
    //-----------------------------------------------------------------------
    // PUBLIC
    //-----------------------------------------------------------------------
    module.exports = stitch = Proteus.create(Driver, {
        ...
        defineTasks: function () {
            if (!this.__tasksDefined__) {
                defineTask("weave");
                defineTask("trace");
                // TypeModel.getAll().map(function (type) {
                //     sake.task("weave", ["weave-" + type.extension]);
                //     sake.task("trace", ["trace-" + type.extension]);
                // });
