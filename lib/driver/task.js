...
(function () {
    ...
    function mapPrereqs (task, args) {
        ...
        sake.log("[mapPrereqs: " + task.name + "]");
        task.prerequisites.forEach(function (name) {
            var prereq = sake.Task.get(name);
            sake.log("[mapping prereq: " + prereq.name + "]");
            if (isNeeded(prereq)) {
                sake.log("[prereq: " + prereq.name + " is needed]");
                sake.log("" + prereq);
                this.splice.apply(
                    this,
        ...
        sake.log("[chain: [" + chain.map(function (t) {
            return t.task.name + "[" + t.index + "]";
        }).join(", ") + "]]");
