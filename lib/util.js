...
(function (exports) {
    ...
    function isRegExp (r) {
        return typeof r === kObject &&
            objToString.call(r) === "[object RegExp]";
    }
    ...
    function splice (listA, idx, count, listB) {
        if (isArray(idx)) {
            listB = idx;
            idx = listA.length;
            count = 0;
        }
        else if (isArray(count)) {
            listB = count;
            count = 0;
        }
        
        listB = isArray(listB) ? listB : slice(arguments, 3);

        return listA.splice.apply(listA, [idx, count].concat(listB));
    }
    ...
    function aliasMethod (name, scope) {
        return function () {
            ...
            if (!fn) {
                throw new Error("No method '" + name + "' defined.");
            }
