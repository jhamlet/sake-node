...
(function (exports) {
     ...
     */
    function runActions () {
        var taskArgs = [this].concat(this.taskArguments),
            queue = new AsyncQueue(),
            queueSetAsync,
            queueClearAsync
        ;
            
        // Add our actions to the queue
        this.actions.forEach(function (fn) {
            queue.add(fn, taskArgs);
        });
        
        // Link our async set and clear into the queue's
        this.on("asyncSet", queueSetAsync = queue.setAsync.bind(queue));
        this.on("asyncCleared", queueClearAsync = queue.clearAsync.bind(queue));
        
        // When all done, clean up and signal we're complete
        queue.on("done", function () {
            this.removeListener("asyncSet", queueSetAsync);
            this.removeListener("asyncCleared", queueClearAsync);
    ...
    module.exports = Task = Model.derive({
         ...
         * @param action {function} additional action for this task
         * @returns {object} Task
         */
