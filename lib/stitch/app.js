...
(function () {
    ...
    module.exports = App = {
        ...
        runCommand: function (cmd, arg) {
            ...
            arg = arg && arg.replace(/^(([\w\d-]+):)?([\w\d-]+)\.(\w+)$/, function () {
                cfgName  = arguments[2];
            ...
            cfg  = opts.config && ConfigModel.get(opts.config);
            bndl = cfg.bundle(opts.bundle, true);
            type = TypeModel.get(opts.type);
            
            switch (cmd) {
                case "trace":
                    console.log(this.compile(cfg, bndl, type));
                    break;
                case "weave":
                    break;
            }
