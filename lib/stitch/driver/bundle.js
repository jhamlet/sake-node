...
(function (exports) {
    ...
    TypeModel.on("created", function (type) {
        console.log("Type " + type.name + " created.");
    });
    
    TypeModel.on("extended", function (type) {
        console.log("Type " + type.name + " extended.");
    });
    
    for (mime in Types) {
        type = Types[mime];
        BundleDriver[type] = bindType(type);
    }
    
    BundleDriver.javascript = bindType("js");
    BundleDriver.stylesheet = bindType("css");
