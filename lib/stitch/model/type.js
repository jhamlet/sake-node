        ...
        self: {
            ...
            getByExtension: function (ext) {
                return this.find(function (type) {
                    return (type.extensions.indexOf(ext) > -1);
                })[0];
            },
        ...
        set extension (ext) {
            ...
            if (util.isArray(ext)) {
                for (i = 0, len = ext.length; i < len; i++) {
                    e = ext[i];
                    if (this.extensions.indexOf(e) < 0) {
                        this.extensions.push(e);
                    }
                }
            }
            else if (this.extensions.indexOf(ext) < 0) {
                this.extensions.push(ext);
            }
            
            TypeModel.emit("extended", this);
        }
        
    });
    
    Proteus.extend(TypeModel, EM.prototype);
