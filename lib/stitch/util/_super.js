...
(function (exports) {
     ...
     * 
     * NOTE: this function depends on ECMAScript 5 features of JavaScript
     * (WebKit, Gecko, no IE), you have to invoke it with the correct 'this'
     * context.
     * 
     * It will climb the prototype chain to find the method with the same
     * name as the one that was called, and call it in the correct scope
     * (the current 'this' object where '_super' is called).
     * 
     * This works for getters and setters, as well as getting overridden
     * plain properties further up the prototype chain.  i.e: if you have
     * objA with a plain property of 'foo' as a number, and objB, with its
     * prototype linked to objA, has a getter, or function, named 'foo',
     * calling '_super' will return the number.
     * 
     * Useage:
     * 
     * You can include in any of your prototypes for classes, or objects
     * created with Ojbect.create:
     * 
     ...
     *      }
     * 
     * Ultimately, directly calling the next object in the chain would be
     * more performant (i.e: inside objB.foo: objA.foo.call(this, arg)).
     * 
     * @method _super
    ...
    exports._super = function _super () {
         ...
         * been named, or if we've been down this route before and applied
         * our own label to the function (since we can not modify the 'name'
        ...
        else if (!scope.__superScope__) {
            // Make sure we advance our prototype scope to the current
            // invoker level if we haven't already
            while (!scope.hasOwnProperty(name)) {
