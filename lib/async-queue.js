...
(function () {
    ...
    module.exports = AsyncQueue = Proteus.Class.derive({
        ...
        init: function (scope) {
            ...
            this.add.apply(this, util.slice(arguments, 1));
        },
        //-------------------------------------------------------------------
        // EVENTS
        //-------------------------------------------------------------------
         ...
         * @returns {AsyncQueue}
         */
        add: function (/*fn1, [args1], fnN, [argsN]*/) {
            ...
            for (; i < len; i++) {
                fn = arguments[i];
                args = util.isArray(arguments[i+1]) ? arguments[++i] : [];
                this.__queue__.push({
                    action: fn,
                    args: args,
                    asyncCount: 0
        ...
        next: function () {
            ...
            obj.action.apply(obj.scope || this.scope, obj.args);
